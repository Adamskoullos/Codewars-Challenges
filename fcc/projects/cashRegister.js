function checkCashRegister(price, cash, cid) {
  const values = [
    // provides the value per unit of each bill/coin type
  ...
  const cashTill = sortedArr.map((arr) => {
    // create array to track the change of value for each bill/coin available for change
    return arr;
  });
  ...
  const names = finalArr.map((name) => {
    // can use to identify if exists in finalArr
    return name[0];
  });
  ...
  const converter = () => {
        ...
        if (obj.val <= changeDue) {
          cashTill.forEach((arr) => {
            if (
              ...
              // Needs to be more than or equal too...this is the bug to sort
              tempArr.push(obj);
            }
          });
        }
    ...
    if (tempArr.length) {
      ...
      // If the bill/coin type of array[0] exists in finalArr
      if (tempArr.length && names.includes(array[0])) {
        ...
        if (changeDue > 0) {
          // If change still owed, run the function again
          console.log(
            "-----------------------running again ",
            changeDue,
            " change still owed"
      ...
      if (tempArr.length && array.length && names.indexOf(array[0]) == -1) {
        ...
        cashTill.forEach((i) => {
          if (i[0] == array[0]) {
            i[1] = i[1] - array[1]; // Credit cashTill------------
            console.log(
              "----------------New bill/coin added and cashTill updated:",
              i
            );
          }
        });
  ...
  console.log("Total change paid to customer: ", finalArr);
